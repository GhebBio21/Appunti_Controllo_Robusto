...
{\LARGE Traduzione in italiano e approfondimento\par}
\vspace{0.5em}
{\Large Mixed sensitivity, scelta dei pesi e limiti da zeri multivariabili\par}
\vfill
\includegraphics[width=0.78\textwidth]{images15/formulation-72-78-001.png}
\vfill
\begin{focusbox}
\textbf{Obiettivo del documento.} Questa parte completa la discussione sulla \emph{mixed sensitivity}: si riscrive la specifica come norma $\mathcal{H}_\infty$ di uno stack di funzioni pesate, si chiarisce il problema $S/KS$, si discutono criteri pratici per scegliere $W_P$, $W_u$ e $W_T$, e si introduce il primo limite fondamentale dovuto agli zeri a parte reale positiva nei sistemi MIMO.
\end{focusbox}
\vfill
{\large Basato sulle slide del corso AA 2025/2026\par}
\end{coverpage}
...
La Lezione 14 ha introdotto il motivo per cui la sola specifica
$$
\|w_P S\|_\infty<1
$$
non basta. Essa controlla bene l'errore, ma non dice tutto sullo sforzo di controllo e sul comportamento ad alta frequenza. La Lezione 15 prende questa osservazione e la trasforma nella formulazione standard di \emph{mixed sensitivity}.

Il filo logico delle slide 72--78 \`e il seguente:
\begin{enumerate}
    \item si impilano pi\`u funzioni di trasferimento pesate in un'unica matrice $N$;
    \item si minimizza la norma $\mathcal{H}_\infty$ di $N$, come fa anche la funzione \texttt{mixsyn} di Matlab;
    \item si guarda in particolare al problema $S/KS$, cio\`e al compromesso fra errore e azione di controllo;
    \item si discutono scelte pratiche per i pesi $W_P$, $W_u$ e $W_T$;
    \item si mostra un esempio in cui cambiare il peso di prestazione modifica tracking e rigetto del disturbo;
    \item si apre il tema dei limiti fondamentali imposti dagli zeri MIMO a parte reale positiva.
\end{enumerate}

\begin{keybox}
\textbf{Idea guida.} La mixed sensitivity non aggiunge formule per complicare il problema: serve a dire contemporaneamente tre cose al controllore. Vogliamo errore piccolo, comando non eccessivo e bassa sensibilit\`a ad alta frequenza. Lo stack $N$ \`e il contenitore unico di queste richieste.
\end{keybox}

\subsection{Lo stack di mixed sensitivity}

La prima slide della lezione mostra lo schema usato per imporre specifiche su pi\`u funzioni di trasferimento. Nel loop SISO di base, con controllore $K$ e processo $G$, si osservano tre segnali pesati:
\begin{itemize}
    \item l'errore, pesato da $w_P$;
    \item l'azione di controllo $u$, pesata da $w_u$;
    \item l'uscita $y$, o equivalentemente la sensibilit\`a complementare, pesata da $w_T$.
\end{itemize}

Le corrispondenti funzioni di trasferimento chiuse sono:
$$
S=\frac{1}{1+GK},
\qquad
T=\frac{GK}{1+GK},
...
KS=\frac{K}{1+GK}.
$$
La specifica mista viene raccolta nella matrice colonna
$$
N=
\begin{bmatrix}
w_P S\\
...
\|N\|_\infty
=
\max_\omega \bar\sigma\bigl(N(j\omega)\bigr)
<1.
$$

Nel caso SISO lo stack $N$ \`e una colonna di scalari complessi. Il suo massimo valore singolare coincide quindi con la norma euclidea della colonna:
$$
\bar\sigma(N)
=
\sqrt{
...
$$
Questo dettaglio \`e importante. Non si sta chiedendo soltanto che ciascun termine sia minore di uno separatamente: si chiede che la combinazione complessiva resti sotto uno a ogni frequenza. Se due termini sono contemporaneamente grandi nella stessa banda, la norma dello stack lo vede.

Il problema di progetto diventa allora
$$
\min_K \|N(K)\|_\infty.
$$
Questo \`e il problema risolto, nella pratica, da funzioni di sintesi come \texttt{mixsyn}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-72-78-001.png}
    \caption{Specifiche miste: errore, uscita e comando vengono pesati e raccolti nello stack $N$.}
\end{figure}

\subsection{Il problema \texorpdfstring{$S/KS$}{S/KS}}

La slide successiva considera una versione pi\`u semplice, ma molto usata, della mixed sensitivity: il problema $S/KS$. In questo caso si controllano solo due canali:
$$
N=
\begin{bmatrix}
W_P S\\
...
\end{bmatrix}.
$$
Il primo termine, $W_P S$, misura la prestazione sull'errore; il secondo, $W_uKS$, misura lo sforzo di controllo. La sintesi cerca un controllore che renda piccolo lo stack completo in norma $\mathcal{H}_\infty$.

La slide ricorda che $S$ \`e la funzione di trasferimento fra il riferimento $r$ e
$$
-e=y-r.
$$
La convenzione di segno non cambia il contenuto della specifica: il punto \`e che $S$ misura quanto l'errore resta sensibile al riferimento e ai disturbi nella banda di interesse.

Nel caso MIMO si sceglie spesso un peso diagonale
$$
W_P=\operatorname{diag}\{w_{P_i}\},
$$
con un peso per ciascun canale principale. Una scelta tipica \`e
$$
w_{P_i}(s)
=
\frac{s/M_i+\omega_{B_i}^\ast}
{s+\omega_{B_i}^\ast A_i},
\qquad
A_i\ll1.
$$
Come nella Lezione 14, $A_i$ impone il livello di errore a bassa frequenza, $\omega_{B_i}^\ast$ impone una banda minima desiderata e $M_i$ limita il picco massimo ammesso della sensibilit\`a.

La slide suggerisce anche una procedura pratica alternativa. Si pu\`o prima progettare un controllore con un altro metodo, poi tracciare il modulo degli elementi diagonali di $S$ e scegliere $w_{P_i}$ come approssimazione razionale di
$$
\frac{1}{S_{ii}(s)}.
$$
L'idea \`e costruire un peso che segua il bound desiderato sul canale corrispondente, ma in una forma razionale utilizzabile dagli algoritmi di sintesi.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-72-78-002.png}
    \caption{Problema $S/KS$: si minimizza lo stack formato da sensibilit\`a pesata e sforzo di controllo pesato.}
\end{figure}

\subsection{Scelta del peso sul comando \texorpdfstring{$W_u$}{Wu} e disturbi espliciti}

La terza slide interpreta il secondo canale dello stack. La funzione
$$
KS
$$
\`e la funzione di trasferimento dal riferimento $r$ all'ingresso di controllo $u$. In un sistema gi\`a scalato, una scelta semplice e ragionevole \`e
$$
W_u=I.
$$
Questa scelta significa che il comando \`e gi\`a espresso in unit\`a normalizzate e che si vuole limitarne direttamente la grandezza.

Una scelta pi\`u informativa consiste nell'usare un peso dinamico del tipo
$$
W_u(s)=\frac{s}{s+\omega_1},
$$
dove $\omega_1$ \`e circa la banda chiusa desiderata. Questo peso \`e piccolo a bassa frequenza e tende a uno ad alta frequenza. In questo modo si evita di penalizzare troppo il comando lento, spesso necessario per inseguire e rigettare disturbi, ma si penalizza maggiormente l'azione di controllo rapida.

La seconda parte della slide riguarda il rigetto dei disturbi. Se il disturbo entra attraverso una dinamica $G_d$, si potrebbe essere tentati di rendere pi\`u ripido il peso $w_{P_i}$ alle basse frequenze. Le slide suggeriscono per\`o una scelta pi\`u pulita: modellare il disturbo esplicitamente nello stack. Si pu\`o scrivere, ad esempio,
$$
N=
\begin{bmatrix}
W_P S & W_P S G_d\\
...
Il significato \`e chiaro: invece di nascondere il disturbo dentro un peso di prestazione troppo artificiale, si dichiara esplicitamente come il disturbo entra nel sistema.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-72-78-003.png}
    \caption{Il peso $W_u$ limita il comando; se il disturbo ha una dinamica propria $G_d$, conviene inserirla esplicitamente nello stack.}
\end{figure}

\subsection{Il ruolo di \texorpdfstring{$T$}{T} e il roll-off ad alta frequenza}

La slide successiva torna alla sensibilit\`a complementare. La funzione $T$ \`e la funzione di trasferimento da $-n$ a $y$, cio\`e descrive quanto il rumore di misura riesce a passare sull'uscita. Per ridurre sensibilit\`a a rumore e incertezza, vogliamo che $T$ sia piccola ad alta frequenza.

Una possibilit\`a diretta \`e aggiungere il termine
$$
W_TT
$$
nello stack $N$, con
$$
W_T=\operatorname{diag}\{w_{T_i}\}.
$$
Il profilo tipico di $w_{T_i}$ \`e opposto a quello del peso di prestazione: piccolo a bassa frequenza, grande ad alta frequenza. Cos\`i il vincolo $\|W_TT\|_\infty<1$ diventa severo proprio dove vogliamo che $T$ decada.

La slide propone anche un approccio pi\`u diretto, basato sullo \emph{shaping} della pianta. Si aggiunge una dinamica ad alta frequenza $W_1(s)$ al modello del processo, ottenendo la pianta sagomata
$$
G_s=G W_1.
$$
Si progetta poi un controllore $\mathcal{H}_\infty$ ottimo $K_s$ per questa pianta sagomata e, alla fine, si include $W_1$ nel controllore:
$$
K=W_1(s)K_s.
$$
Lo scopo \`e forzare il roll-off desiderato gi\`a nella pianta vista dalla procedura di sintesi.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-72-78-004.png}
    \caption{Per controllare rumore e incertezza ad alta frequenza si pu\`o pesare $T$ oppure sagomare direttamente la pianta con una dinamica $W_1$.}
\end{figure}

\subsection{Esempio di progetto \texorpdfstring{$S/KS$}{S/KS}}

Le slide propongono poi un esempio SISO. Il processo nominale e la dinamica del disturbo sono
$$
G(s)=
\frac{200}{10s+1}\,
\frac{1}{(0.05s+1)^2},
\qquad
G_d(s)=\frac{100}{10s+1}.
$$
Si considera una sintesi mixed sensitivity del tipo $S/KS$:
$$
N=
\begin{bmatrix}
w_P S\\
...
Si sceglie inizialmente
$$
w_u=1,
$$
cos\`i il comando normalizzato deve restare minore di uno in modulo. Per la prestazione si usa prima
$$
w_{P1}(s)
=
\frac{s/M+\omega_B^\ast}
{s+\omega_B^\ast A},
\qquad
M=1.5,\quad
\omega_B^\ast=10,\quad
A=10^{-4}.
$$
Le slide annotano che questa scelta produce una cattiva risposta al disturbo. Il motivo \`e che il peso impone certe prestazioni su $S$, ma non rende abbastanza aggressivo il loop nella banda in cui il disturbo $G_d$ \`e rilevante.

Per ottenere guadagni pi\`u elevati a bassa frequenza, si prova allora un peso con pendenza pi\`u ripida:
$$
w_{P2}(s)
=
\frac{\left(s/M^{1/2}+\omega_B^\ast\right)^2}
{\left(s+\omega_B^\ast A^{1/2}\right)^2},
\qquad
M=1.5,\quad
\omega_B^\ast=10,\quad
A=10^{-4}.
$$
Questa scelta modifica il bound $1/w_P$ su $S$ e forza una sensibilit\`a pi\`u piccola alle basse frequenze, migliorando il rigetto del disturbo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-72-78-005.png}
    \caption{Esempio SISO: due scelte del peso di prestazione producono comportamenti diversi rispetto al disturbo.}
\end{figure}

\subsection{Lettura dei risultati dell'esempio}

La slide successiva mostra tre grafici. Il primo confronta le risposte di tracking, il secondo le risposte al disturbo, il terzo mostra i moduli di $S_1$, $S_2$ e dei bound $1/w_{P1}$, $1/w_{P2}$.

La lettura qualitativa \`e questa:
\begin{itemize}
    \item con il primo peso, la risposta di tracking \`e regolare, ma il rigetto del disturbo \`e lento e poco soddisfacente;
    \item con il secondo peso, la risposta al disturbo migliora molto, perch\'e la sensibilit\`a \`e costretta a essere pi\`u piccola alle basse frequenze;
    \item il prezzo \`e un tracking pi\`u aggressivo, con maggiore sovraelongazione.
\end{itemize}

Il grafico in frequenza chiarisce il motivo. Il secondo bound $1/w_{P2}$ \`e pi\`u severo nella regione di bassa frequenza e quindi obbliga il progetto a ridurre di pi\`u $S$. Questo aumenta il guadagno d'anello nella banda utile per il rigetto del disturbo.

Il messaggio didattico \`e importante: cambiare un peso non \`e un'operazione cosmetica. Significa cambiare il compromesso fisico fra inseguimento, rigetto dei disturbi, picchi dinamici e sforzo di controllo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-72-78-006.png}
    \caption{Risultati dell'esempio: il peso pi\`u ripido migliora il rigetto del disturbo, ma rende il tracking pi\`u aggressivo.}
\end{figure}

\subsection{Limiti imposti dagli zeri MIMO a parte reale positiva}

L'ultima slide del blocco cambia argomento e apre un tema fondamentale: le limitazioni imposte dagli zeri a parte reale positiva nei sistemi multivariabili. Nei sistemi SISO sappiamo gi\`a che zeri nel semipiano destro limitano prestazioni e robustezza. La slide dice che lo stesso accade nei sistemi MIMO, con una difficolt\`a in pi\`u: la presenza degli zeri pu\`o non essere evidente guardando i singoli elementi della matrice $G(s)$.

Nel caso MIMO, uno zero \`e definito come un valore
$$
s=z
$$
per cui la matrice di trasferimento $G(s)$ perde rango:
$$
\operatorname{rank}G(z)<\operatorname{rank}G(s)
\quad
\text{genericamente}.
$$
In modo equivalente, esiste una direzione di ingresso non nulla $v_z$ tale che
$$
G(z)v_z=0.
$$
Questa direzione \`e detta direzione dello zero: \`e una combinazione di ingressi lungo cui, alla frequenza complessa $z$, la matrice $G(z)$ ha guadagno nullo.

Questo punto \`e molto pi\`u sottile del caso SISO. In SISO uno zero si vede direttamente come radice del numeratore. In MIMO, invece, nessun singolo elemento della matrice deve necessariamente annullarsi in modo evidente; ci\`o che si annulla \`e una combinazione direzionale di ingressi e uscite. Per questo le limitazioni multivariabili devono essere lette con strumenti direzionali, non solo canale per canale.

\begin{focusbox}
\textbf{Idea da ricordare.} Uno zero MIMO non \`e semplicemente ``uno zero di un elemento'' di $G(s)$. \`E una perdita di rango della matrice di trasferimento, quindi una direzione in cui il sistema non riesce a trasmettere segnale. Se tale zero \`e nel semipiano destro, impone limiti fondamentali alle prestazioni ottenibili in quella direzione.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-72-78-007.png}
    \caption{Gli zeri MIMO si definiscono tramite perdita di rango di $G(z)$ e introducono limitazioni fondamentali al controllo.}
\end{figure}

\subsection{Ritorno agli zeri RHP multivariabili}

Riprendiamo l'esempio appena introdotto sugli zeri RHP multivariabili. La pianta \`e
$$
G(s)=
\frac{1}{(0.2s+1)(s+1)}
\begin{bmatrix}
...
\end{bmatrix}.
$$
Se si applica un gradino al solo primo ingresso, cio\`e
$$
u=
\begin{bmatrix}
1\\
...
\end{bmatrix},
$$
la risposta non rivela in modo immediato la presenza di uno zero RHP. Lo stesso accade applicando un gradino al solo secondo ingresso,
$$
u=
\begin{bmatrix}
0\\
...
\end{bmatrix}.
$$
Le due risposte mostrano interazione fra i canali, ma non mostrano chiaramente una risposta inversa come ci si aspetterebbe da uno zero a parte reale positiva nel caso SISO.

Il punto \`e che lo zero non vive necessariamente su un singolo canale. In questo esempio lo zero RHP compare quando si eccita una particolare combinazione di ingressi. Infatti, considerando la direzione
$$
u=
\begin{bmatrix}
1\\
...
\end{bmatrix},
$$
la risposta mette in evidenza l'effetto dello zero: una delle uscite mostra una risposta inversa, mentre l'altra pu\`o perfino annullarsi per quella particolare combinazione.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-79-84-001.png}
    \caption{Risposta in anello aperto ai singoli ingressi: lo zero RHP multivariabile non si riconosce guardando solo i canali separati.}
\end{figure}

\subsection{Direzioni dello zero e perdita di rango}

La pianta ha uno zero a parte reale positiva in
$$
z=0.5.
$$
Infatti, in $s=0.5$ la matrice di trasferimento perde rango. La slide scrive la decomposizione SVD di $G(0.5)$ nella forma
$$
G(0.5)
=
\frac{1}{1.65}
...
\end{bmatrix}^{H}.
$$
Il valore singolare nullo identifica la direzione di ingresso che viene annullata dalla pianta in corrispondenza dello zero. Dalla matrice $V$ si ricava
$$
v_z=
\begin{bmatrix}
0.71\\
...
\end{bmatrix},
$$
mentre dalla matrice $U$ si ricava la direzione di uscita associata,
$$
u_z=
\begin{bmatrix}
0.89\\
...
\end{bmatrix}.
$$
Quindi lo zero non \`e associato a un solo ingresso o a una sola uscita: coinvolge entrambe le combinazioni.

Questa \`e la differenza essenziale rispetto al caso SISO. In SISO lo zero \`e una radice del numeratore e il suo effetto \`e visibile direttamente nel canale. In MIMO, invece, lo zero pu\`o essere nascosto rispetto agli esperimenti canale-per-canale, perch\'e riguarda una direzione nello spazio degli ingressi e una direzione nello spazio delle uscite.

\begin{focusbox}
\textbf{Lettura geometrica.} Dire che $G(z)$ perde rango significa dire che esiste una direzione non nulla $v_z$ tale che $G(z)v_z=0$. In quella direzione il sistema non riesce a trasmettere segnale. Se $z$ \`e nel semipiano destro, questa perdita direzionale impone limiti fondamentali alla risposta chiusa.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-79-84-002.png}
    \caption{SVD di $G(0.5)$: il valore singolare nullo rivela la direzione dello zero RHP multivariabile.}
\end{figure}

\subsection{Progetto \texorpdfstring{$S/KS$}{S/KS} per studiare l'effetto dello zero}

Per capire come lo zero RHP influenza il sistema in anello chiuso, le slide considerano una sintesi $\mathcal{H}_\infty$ del problema pesato $S/KS$:
$$
N=
\begin{bmatrix}
W_P S\\
...
\end{bmatrix}.
$$
Qui $S$ misura la sensibilit\`a, quindi l'errore e la risposta ai disturbi; $KS$ misura l'azione di controllo generata dal riferimento o dai segnali esterni. L'obiettivo \`e minimizzare
$$
\|N(K)\|_\infty
$$
rispetto ai controllori stabilizzanti $K$.

Nel caso dell'esempio si sceglie
$$
W_u=I,
\qquad
W_P=
...
A_i=10^{-4}.
$$
Come nelle lezioni precedenti, $M_i$ limita il picco ammesso della sensibilit\`a, $\omega_{B_i}^{\ast}$ impone una richiesta di banda sul canale $i$ e $A_i$ fissa l'errore desiderato a bassa frequenza.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-79-84-003.png}
    \caption{Problema $S/KS$ usato per valutare l'effetto dello zero RHP sulla risposta chiusa.}
\end{figure}

\subsection{Tre scelte di progetto e spostamento dell'effetto dello zero}

Le slide confrontano tre scelte dei pesi:
$$
\text{Design 1:}\qquad
M_1=M_2=1.5,
\qquad
\omega_{B1}^{\ast}=\omega_{B2}^{\ast}=\frac{z}{2}=0.25,
$$
$$
\text{Design 2:}\qquad
M_1=M_2=1.5,
\qquad
\omega_{B1}^{\ast}=0.25,\quad
\omega_{B2}^{\ast}=25,
$$
$$
\text{Design 3:}\qquad
M_1=M_2=1.5,
\qquad
\omega_{B1}^{\ast}=25,\quad
\omega_{B2}^{\ast}=0.25.
$$
Nel primo progetto si chiede la stessa banda ai due canali. Nel secondo si rende molto pi\`u severa la richiesta sul secondo canale. Nel terzo si fa l'opposto: si rende molto pi\`u severa la richiesta sul primo canale.

Il risultato importante \`e che, in un sistema MIMO, l'effetto deteriorante dello zero RHP pu\`o spesso essere spostato da un'uscita all'altra. Non significa che il limite sparisca: significa che si sceglie dove pagare il prezzo. Con il Design 2 si ottiene una risposta migliore in una direzione, accettando il peggioramento nell'altra. Con il Design 3 si pu\`o ottenere una risposta eccellente su $y_1$, senza risposta inversa evidente, ma al prezzo di una risposta molto scadente su $y_2$.

Le slide riportano anche un dato numerico: per il Design 3 si ottiene
$$
\|N\|_\infty=6.73,
$$
mentre per il Design 2 il valore era circa
$$
\|N\|_\infty=2.92.
$$
Questo conferma che il terzo progetto compra il miglioramento di un canale con un peggioramento globale pi\`u severo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-79-84-004.png}
    \caption{Tre scelte dei pesi: cambiando le bande richieste ai due canali si decide dove far ricadere l'effetto dello zero RHP.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-79-84-005.png}
    \caption{Confronto fra progetti: i valori singolari di $S$ e le risposte al riferimento mostrano il compromesso direzionale imposto dallo zero.}
\end{figure}

\subsection{Zeri pinned e limite della norma \texorpdfstring{$\mathcal{H}_\infty$}{Hinf}}

La prima osservazione delle slide \`e che lo spostamento dell'effetto dello zero non \`e sempre possibile. In alcuni sistemi MIMO lo zero \`e associato a uno specifico canale di uscita e non si pu\`o trasferire il suo effetto su un altro canale. In questo caso si parla di \emph{pinned zero}. Il termine indica uno zero ``inchiodato'' a una certa uscita: il progettista non pu\`o scegliere liberamente dove far comparire la risposta inversa o il degrado prestazionale.

La seconda osservazione \`e pi\`u sottile e riguarda la norma $\mathcal{H}_\infty$. Nel Design 2 si ottiene un miglioramento molto grande nella direzione buona, cio\`e quella associata a $\underline{\sigma}(S)$, a fronte di un peggioramento minore nella direzione cattiva, associata a $\bar{\sigma}(S)$. Tuttavia la norma $\mathcal{H}_\infty$ guarda solo il massimo valore singolare, quindi vede soprattutto la direzione peggiore.

Questo \`e un punto concettuale importante. La norma $\mathcal{H}_\infty$ \`e molto utile per garantire prestazioni worst-case, ma pu\`o rendere difficile valorizzare miglioramenti importanti nelle direzioni non peggiori. Per questo, nei problemi multivariabili, non basta guardare un solo numero: bisogna anche interpretare le direzioni singolari.

\begin{keybox}
\textbf{Messaggio da ricordare.} Gli zeri RHP MIMO impongono limiti direzionali. I pesi permettono di distribuire il prezzo fra i canali, ma non eliminano il vincolo fisico. Inoltre la norma $\mathcal{H}_\infty$ sintetizza tutto nel caso peggiore: ottima per garanzie robuste, meno descrittiva dei compromessi fra direzioni.
\end{keybox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images15/formulation-79-84-006.png}
    \caption{Osservazioni finali: pinned zeros e limite della norma $\mathcal{H}_\infty$ come misura basata sulla direzione peggiore.}
\end{figure}


\subsection{Sintesi conclusiva della lezione}

La Lezione 15 completa il passaggio dalla scelta qualitativa dei pesi alla formulazione operativa del problema di sintesi. La mixed sensitivity costruisce uno stack $N$ che raccoglie pi\`u richieste:
\begin{itemize}
    \item $W_P S$ per controllare l'errore e la prestazione;
    \item $W_u K S$ per limitare l'azione di controllo;
    \item $W_T T$ per imporre roll-off e attenuazione del rumore ad alta frequenza.
\end{itemize}

Nel problema $S/KS$ ci si concentra sui primi due canali, scegliendo $W_P$ per sagomare la sensibilit\`a e $W_u$ per rendere fisicamente ragionevole il comando. Se i disturbi hanno una dinamica propria, \`e spesso meglio inserirla esplicitamente nello stack invece di forzare artificialmente il peso di prestazione.

Gli esempi mostrano che i pesi cambiano davvero il comportamento del sistema: un peso pi\`u severo a bassa frequenza migliora il rigetto dei disturbi, ma pu\`o rendere il tracking pi\`u aggressivo. Il blocco finale sugli zeri RHP multivariabili completa il messaggio: non tutte le prestazioni desiderate sono realizzabili, e nei sistemi MIMO i limiti sono spesso direzionali. I pesi possono decidere dove pagare il prezzo, ma non possono cancellare lo zero.

\begin{keybox}
\textbf{Messaggio operativo.} La sintesi $\mathcal{H}_\infty$ non sceglie da sola che cosa sia importante: lo leggiamo nei pesi. Per questo $W_P$, $W_u$ e $W_T$ devono essere scelti come specifiche di progetto, non come parametri decorativi.
\end{keybox}
...
Questa parte del documento \`e una traduzione e rielaborazione estesa delle slide 72--84 del PDF ``Formulation of Control Problems'', caricato dall'utente come materiale di partenza nella directory \texttt{Slide}.

% --- FRONTESPIZIO LEZIONE 16 ---
\begin{coverpage}
\centering
{\Huge\bfseries Controllo Robusto\par}
\vspace{0.4em}
{\LARGE Traduzione in italiano e approfondimento\par}
\vspace{0.5em}
{\Large Formulazione generale del problema di controllo, LFT e pianta generalizzata\par}
\vfill
\includegraphics[width=0.78\textwidth]{images16/h2hinf-01-24-001.png}
\vfill
\begin{focusbox}
\textbf{Obiettivo del documento.} Questa parte apre la formulazione generale del problema di controllo. L'idea \`e passare da schemi specifici di retroazione alla forma standard $P$-$K$: un impianto generalizzato $P$, un controllore $K$, ingressi esogeni $w$, uscite di prestazione $z$, segnali misurati $v$ e comandi $u$. Per descrivere in modo compatto questa chiusura di anello si introduce il linguaggio delle trasformazioni frazionarie lineari, o LFT.
\end{focusbox}
\vfill
{\large Basato sulle slide del corso AA 2025/2026\par}
\end{coverpage}
...
La Lezione 16 apre il capitolo sulla formulazione generale del problema di controllo. Dopo aver costruito diversi stack di mixed sensitivity nelle lezioni precedenti, ora si introduce una forma unica capace di rappresentare quei problemi in modo sistematico: la forma $P$-$K$.

La lezione ha tre blocchi principali:
\begin{enumerate}
    \item formulazione generale del problema di controllo tramite ingressi esogeni $w$, uscite di prestazione $z$, misure $v$ e comandi $u$;
    \item introduzione delle trasformazioni frazionarie lineari, o LFT, come linguaggio compatto per chiudere interconnessioni;
    \item costruzione della pianta generalizzata $P$ in esempi a uno e due gradi di libert\`a, fino alla mappa chiusa $N=F_\ell(P,K)$.
\end{enumerate}

\begin{keybox}
\textbf{Idea guida.} Uno schema di controllo robusto si pu\`o sempre leggere come una mappa chiusa da ingressi esterni $w$ a uscite pesate $z$. La domanda di progetto diventa: scegliere $K$ in modo che la mappa chiusa $N(K)$ abbia norma piccola. Le LFT sono il modo compatto per scrivere proprio questa dipendenza di $N$ da $K$.
\end{keybox}

\subsection{Formulazione generale del problema di controllo}

Finora abbiamo scritto a mano diversi stack:
$$
\begin{bmatrix}
W_P S\\
W_uKS
...
\end{bmatrix},
$$
oppure matrici chiuse da disturbi, riferimenti e rumore verso errori e comandi. La domanda naturale \`e: esiste un modo unico per rappresentare tutti questi casi?

La risposta \`e la forma $P$-$K$, introdotta da Doyle. Si costruisce un impianto generalizzato $P$ con quattro famiglie di segnali:
\begin{itemize}
    \item $w$: ingressi esogeni, eventualmente pesati, come riferimenti, disturbi e rumore;
    \item $z$: uscite esogene o uscite di prestazione, eventualmente pesate;
    \item $u$: segnali di controllo prodotti dal controllore;
    \item $v$: segnali misurati disponibili al controllore.
\end{itemize}
Il controllore riceve $v$ e genera $u$. L'interconnessione chiusa produce una mappa
$$
N(K):w\mapsto z.
$$
Il problema di sintesi consiste nel trovare un controllore stabilizzante $K$ che renda piccola una norma di questa mappa, per esempio
$$
\|N(K)\|_2
\qquad\text{oppure}\qquad
\|N(K)\|_\infty.
...
\begin{focusbox}
\textbf{Forma standard.} La formulazione $P$-$K$ separa due cose: $P$ contiene il modello, i pesi e la geometria del problema; $K$ \`e l'oggetto da progettare. Una volta chiuso l'anello, tutto ci\`o che interessa alla sintesi \`e la mappa $N(K)$ da $w$ a $z$.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-002.png}
    \caption{Forma generale $P$-$K$: il controllore usa $v$ per generare $u$ e ridurre l'effetto di $w$ su $z$.}
\end{figure}

\subsection{Trasformazioni frazionarie lineari: idea scalare}

Per descrivere in modo compatto la dipendenza di $N$ da $K$, le slide introducono le \emph{linear fractional transformations}, abbreviate LFT. L'idea parte dal caso scalare. Una trasformazione del tipo
$$
F(s)=\frac{a+bs}{c+ds},
\qquad
a,b,c,d\in\mathbb{C},
$$
\`e detta trasformazione frazionaria lineare. Se $c\neq0$, essa pu\`o essere riscritta nella forma
$$
F(s)=\alpha+\beta s(1-\gamma s)^{-1},
$$
per opportuni $\alpha,\beta,\gamma\in\mathbb{C}$.

Questa seconda forma \`e quella pi\`u utile per il controllo, perch\'e assomiglia a una mappa nominale pi\`u una correzione che passa attraverso una retroazione. In pratica, il termine $(1-\gamma s)^{-1}$ anticipa gi\`a il denominatore che nasce quando si chiude un loop.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-003.png}
    \caption{LFT nel caso scalare: una funzione razionale di primo grado pu\`o essere vista come chiusura frazionaria.}
\end{figure}

\subsection{LFT inferiori e superiori}

Il caso matriciale generalizza l'idea precedente. Sia $M$ una matrice complessa partizionata come
$$
M=
\begin{bmatrix}
M_{11} & M_{12}\\
...
\end{bmatrix}.
$$
Si definiscono due modi di chiudere una parte della matrice con un blocco esterno $\Delta$.

La \emph{lower LFT} rispetto a $\Delta_\ell$ \`e
$$
F_\ell(M,\Delta_\ell)
=
M_{11}
+M_{12}\Delta_\ell
\left(I-M_{22}\Delta_\ell\right)^{-1}
M_{21},
$$
purch\'e l'inversa esista. Si chiama ``lower'' perch\'e, nella rappresentazione a blocchi, si chiude il loop sulla parte bassa dell'interconnessione.

La \emph{upper LFT} rispetto a $\Delta_u$ \`e invece
$$
F_u(M,\Delta_u)
=
M_{22}
+M_{21}\Delta_u
\left(I-M_{11}\Delta_u\right)^{-1}
M_{12},
$$
anche qui sotto l'ipotesi che l'inversa esista. La matrice $M$ viene chiamata \emph{coefficient matrix}: contiene il sistema nominale e il modo in cui il blocco esterno si collega al resto.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-004.png}
    \caption{Definizione di lower LFT: si chiude la parte inferiore della matrice partizionata con il blocco $\Delta_\ell$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-005.png}
    \caption{Definizione di upper LFT: si chiude la parte superiore dell'interconnessione con il blocco $\Delta_u$.}
\end{figure}

\subsection{Interpretazione delle LFT}

Le LFT non sono solo una notazione compatta. La loro interpretazione fisica \`e molto utile:
\begin{itemize}
    \item $M_{11}$ rappresenta spesso la mappa nominale;
    \item $\Delta$ rappresenta un'incertezza, una dinamica aggiuntiva oppure il controllore;
    \item $M_{12}$, $M_{21}$ e $M_{22}$ descrivono come $\Delta$ entra e rientra nell'interconnessione.
\end{itemize}

Per esempio, se $\Delta$ rappresenta un'incertezza di modello, la LFT descrive tutte le piante perturbate ottenute chiudendo l'incertezza attorno al modello nominale. Se invece $\Delta=K$, la stessa formula descrive il sistema chiuso ottenuto collegando il controllore alla pianta generalizzata.

Questa \`e la ragione per cui le LFT sono cos\`i centrali nel controllo robusto: permettono di trattare con lo stesso linguaggio sia l'incertezza sia la sintesi del controllore.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-006.png}
    \caption{Le parole ``lower'' e ``upper'' derivano dal loop che viene chiuso nel diagramma a blocchi.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-007.png}
    \caption{Interpretazione delle LFT: una mappa nominale viene perturbata o chiusa con un blocco esterno.}
\end{figure}

\subsection{Esempi di LFT}

Le slide mostrano due esempi importanti. Il primo riguarda i polinomi. Un polinomio
$$
p(s)=a_0+a_1s+\cdots+a_ns^n
$$
pu\`o essere scritto come LFT rispetto a $sI_n$. L'idea \`e costruire una matrice $M$ con una struttura a catena, simile a una realizzazione in forma compagna, in modo che chiudendo il blocco $sI_n$ si generino le potenze successive di $s$.

Il messaggio non \`e che convenga rappresentare ogni polinomio in questo modo, ma che le LFT sono abbastanza generali da contenere anche oggetti algebrici semplici. Lo stesso vale per polinomi multidimensionali, ad esempio in variabili $\delta_1,\delta_2$, che possono essere scritti come LFT rispetto a una matrice diagonale a blocchi
$$
\Delta=
\begin{bmatrix}
\delta_1 I & 0\\
...
\end{bmatrix}.
$$
Questo sar\`a naturale quando le $\delta_i$ rappresentano parametri incerti.

Il secondo esempio \`e ancora pi\`u importante per il controllo. Se un sistema ha realizzazione in spazio di stato
$$
\dot{x}=Ax+Bu,
\qquad
y=Cx+Du,
$$
allora la sua matrice di trasferimento \`e
$$
G(s)=D+C(sI-A)^{-1}B.
$$
Questa espressione pu\`o essere vista come una upper LFT della matrice
$$
\begin{bmatrix}
A & B\\
C & D
\end{bmatrix}
$$
rispetto al blocco $\Delta=s^{-1}I$. Infatti
$$
D+C\,s^{-1}I\left(I-A\,s^{-1}I\right)^{-1}B
=
D+C(sI-A)^{-1}B.
$$
Quindi anche una realizzazione dinamica ordinaria \`e una LFT.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-008.png}
    \caption{Esempio: un polinomio pu\`o essere rappresentato come LFT delle sue indeterminate.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-009.png}
    \caption{Esempio centrale: una realizzazione in spazio di stato produce la funzione di trasferimento come LFT.}
\end{figure}

\subsection{Costruire la pianta generalizzata \texorpdfstring{$P$}{P}}

Le routine di sintesi $\mathcal{H}_2$ e $\mathcal{H}_\infty$ in Matlab assumono che il problema sia gi\`a scritto nella forma generale. Questo significa che bisogna identificare:
$$
w,\qquad z,\qquad v,\qquad u.
$$
Una volta scelti questi segnali, si costruisce $P$ come sistema \emph{open-loop} che manda
$$
\begin{bmatrix}
w\\
u
...
\end{bmatrix}.
$$
La regola pratica delle slide \`e: per costruire $P$, bisogna rompere tutti i loop che entrano ed escono dal controllore $K$. In altre parole, $P$ non contiene $K$; contiene solo tutto ci\`o che sta attorno a $K$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-010.png}
    \caption{Per ottenere $P$ si identificano $w,z,v,u$ e si apre l'anello nel punto occupato dal controllore.}
\end{figure}

\subsection{Esempio intermedio: schema Matlab con pesi}

Prima dello schema classico a un grado di libert\`a, le slide mostrano un esempio molto operativo, pensato proprio per il collegamento con Matlab. L'obiettivo \`e trasformare uno schema a blocchi con pesi in una forma $P$-$K$.

Gli ingressi esogeni sono
$$
w=
\begin{bmatrix}
d\\
...
\end{bmatrix},
$$
mentre le uscite di prestazione sono
$$
z=
\begin{bmatrix}
v\\
...
\end{bmatrix}.
$$
Qui $v$ \`e l'uscita pesata da $W_2$, mentre $u_f$ \`e il comando filtrato da $W_1$. Il segnale di misura che torna al controllore \`e indicato con $y$. Aprendo l'anello nel punto del controllore, la slide scrive
$$
\begin{bmatrix}
z\\
y
...
\end{bmatrix}.
$$
La riga superiore dice che il disturbo $d$ e il comando $u$ attraversano la stessa dinamica $G$ e poi il peso $W_2$; la seconda riga pesa direttamente il comando con $W_1$; l'ultima riga descrive il ramo di misura filtrato da $F$ e con segno coerente con la retroazione.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-011.png}
    \caption{Esempio di costruzione della pianta generalizzata da uno schema pesato, pronto per essere collegato in Matlab.}
\end{figure}

\subsection{Esempio: feedback a un grado di libert\`a}

Consideriamo la configurazione classica a un grado di libert\`a. Gli ingressi esogeni sono:
$$
w=
\begin{bmatrix}
d\\
...
\end{bmatrix},
$$
dove $d$ \`e un disturbo additivo sull'uscita, $r$ \`e il riferimento e $n$ \`e il rumore di misura. Si scelgono
$$
z=e=y-r,
\qquad
v=r-y_m=r-y-n.
...
{}-Gu.
$$
La pianta generalizzata \`e quindi
$$
\begin{bmatrix}
z\\
v
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-012.png}
    \caption{Scelta dei segnali nel feedback a un grado di libert\`a: $w=[d\ r\ n]^T$, $z=e$, $v=r-y_m$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-013.png}
    \caption{Costruzione esplicita di $P$ per lo schema a un grado di libert\`a.}
\end{figure}

\subsection{Chiudere il loop: da \texorpdfstring{$P$}{P} a \texorpdfstring{$N$}{N}}

Una volta progettato il controllore, per analizzare la prestazione chiusa si assorbe $K$ nell'interconnessione. Si ottiene
$$
z=Nw.
$$
Per calcolare $N$, si parte dalla partizione
$$
\begin{bmatrix}
z\\
v
...
\end{bmatrix},
$$
e si usa l'equazione del controllore
$$
u=Kv.
$$
Eliminando $u$ e $v$ si ricava
$$
N
=
P_{11}
+P_{12}K(I-P_{22}K)^{-1}P_{21}
=F_\ell(P,K).
$$
Questa \`e una lower LFT di $P$ rispetto a $K$. Il segno meno dentro l'inversa nasce dalla convenzione usata per scrivere la retroazione come chiusura positiva della LFT.

Applicando questa formula all'esempio precedente:
$$
N
=
\begin{bmatrix}
...
z=Nw=S_od-S_or-T_on.
$$
Questa formula dice esattamente ci\`o che gi\`a conosciamo dalle lezioni precedenti: il disturbo in uscita e il riferimento passano attraverso $S_o$, mentre il rumore di misura passa attraverso $T_o$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-014.png}
    \caption{La mappa chiusa $N$ si ottiene eliminando $u$ e $v$ e chiudendo la lower LFT $F_\ell(P,K)$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-015.png}
    \caption{La chiusura di $K$ attorno a $P$ \`e una lower LFT; con la convenzione della slide compare il termine $(I-P_{22}K)^{-1}$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-016.png}
    \caption{Per il feedback a un grado di libert\`a si recupera $N=[S_o\ -S_o\ -T_o]$.}
\end{figure}

\subsection{Problema stacked \texorpdfstring{$S/T/KS$}{S/T/KS}}

Le slide tornano poi al problema di mixed sensitivity nella forma pi\`u completa. Si vogliono limitare:
\begin{enumerate}
    \item $\bar{\sigma}(S)$, per la prestazione;
    \item $\bar{\sigma}(T)$, per robustezza e attenuazione del rumore;
    \item $\bar{\sigma}(KS)$, per penalizzare ingressi di controllo troppo grandi.
\end{enumerate}
Queste richieste si raccolgono nello stack
$$
N(K)=
\begin{bmatrix}
W_uKS\\
...
Per ottenere questo stack dalla forma generale, si costruisce una pianta $P$ che mappa
$$
\begin{bmatrix}
w\\
u
...
\end{bmatrix}.
$$
La partizione corrispondente \`e
$$
P_{11}=
\begin{bmatrix}
0\\
...
\end{bmatrix}.
$$
I segni meno sui primi due canali non cambiano la norma, ma sono utili per controllare la coerenza algebrica della chiusura.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-017.png}
    \caption{Problema stacked $S/T/KS$: prestazione, robustezza e comando vengono raccolti in un unico $N(K)$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-018.png}
    \caption{Costruzione della pianta generalizzata per ottenere lo stack $S/T/KS$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-019.png}
    \caption{Partizione di $P$ nello stack $S/T/KS$: i blocchi $P_{11}$, $P_{12}$, $P_{21}$ e $P_{22}$ devono avere dimensioni compatibili con $K$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-020.png}
    \caption{Chiusura dello stack tramite LFT: si recuperano $W_uKS$, $W_TT$ e $W_PS$.}
\end{figure}

\subsection{Esempio a due gradi di libert\`a}

L'ultimo esempio delle slide considera uno schema a due gradi di libert\`a. Gli ingressi esogeni sono
$$
w=
\begin{bmatrix}
d\\
...
\end{bmatrix},
$$
mentre le uscite di prestazione e i segnali disponibili al controllore sono
$$
z=
\begin{bmatrix}
y-r\\
...
\end{bmatrix}.
$$
Il controllore agisce su due informazioni: il riferimento $r$ e il segnale misurato $y_m$. Scrivendo
$$
K=
\begin{bmatrix}
K_1 & K_2
...
u=K_1r+K_2y_m.
$$
Lo schema comprende anche una dinamica del disturbo $G_d$, per cui
$$
y=Gu+G_dd,
\qquad
y_m=y+n.
...
Per costruire $P$ si apre il loop nel punto del controllore. La relazione fra
$$
\begin{bmatrix}
w\\
u
...
\end{bmatrix}.
$$
La slide insiste su un aspetto essenziale: anche in questo caso $P$ non contiene il controllore $K$. Contiene solo la pianta, la dinamica del disturbo, i sommatori e le scelte di segnali.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-021.png}
    \caption{Esempio a due gradi di libert\`a: si scelgono $w$, $z$ e $v$ e si chiede di costruire $P$ e $N$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-022.png}
    \caption{Pianta generalizzata per lo schema a due gradi di libert\`a: il controllore \`e escluso da $P$.}
\end{figure}

\subsection{Mappa chiusa nello schema a due gradi di libert\`a}

La mappa chiusa pu\`o essere ricavata direttamente dal diagramma oppure tramite
$$
N=F_\ell(P,K).
$$
Assumendo
...
\end{bmatrix}.
$$
Questa \`e la matrice $N$ cercata.

Il risultato mostra bene il senso dei due gradi di libert\`a. Il blocco $K_1$ agisce direttamente sul riferimento, quindi pu\`o modellare il tracking; il blocco $K_2$ chiude il feedback sulla misura e influenza stabilit\`a, rigetto dei disturbi e sensibilit\`a al rumore. La forma generale $P$-$K$ permette di tenere insieme questi due ruoli senza cambiare formalismo.

Per usare la formula LFT, si partiziona $P$ come
$$
P_{11}=
\begin{bmatrix}
G_d & -I & 0\\
...
\end{bmatrix}.
$$
Inserendo questi blocchi in
$$
N=P_{11}+P_{12}K(I-P_{22}K)^{-1}P_{21},
$$
si ottiene la stessa mappa chiusa trovata direttamente dal diagramma.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-023.png}
    \caption{Mappa chiusa dello schema a due gradi di libert\`a e partizione di $P$ per usare la LFT.}
\end{figure}

\subsection{Problema inverso: ricavare \texorpdfstring{$P$}{P} da \texorpdfstring{$N$}{N}}

L'ultima slide affronta il problema inverso. A volte si conosce gi\`a la mappa chiusa desiderata $N(K)$ e si vuole trovare una pianta generalizzata $P$ tale che
$$
N=F_\ell(P,K)
=
P_{11}+P_{12}K(I-P_{22}K)^{-1}P_{21}.
$$
In generale il modo pi\`u pulito resta costruire $P$ dal diagramma a blocchi. Se per\`o si vuole partire da $N$, le slide propongono una procedura pratica:
\begin{enumerate}
    \item porre $K=0$ in $N$ per ottenere $P_{11}$;
    \item definire $Q=N-P_{11}$;
    \item riscrivere $Q$ in modo che ogni termine abbia un fattore comune
    $$
    R=K(I-P_{22}K)^{-1};
    $$
    \item usare la fattorizzazione
    $$
    Q=P_{12}RP_{21}
    $$
    per ricavare $P_{12}$, $P_{22}$ e $P_{21}$ per ispezione.
\end{enumerate}
Questa procedura non sostituisce la comprensione fisica dello schema, ma aiuta quando lo stack $N$ \`e gi\`a stato scritto e si vuole tradurlo in una forma compatibile con le routine di sintesi.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images16/h2hinf-01-24-024.png}
    \caption{Procedura inversa: partire da $N(K)$ e ricostruire una possibile pianta generalizzata $P$.}
\end{figure}

\subsection{Sintesi conclusiva della lezione}

La Lezione 16 introduce il linguaggio standard con cui si scrivono i problemi di sintesi robusta. Il punto centrale \`e la struttura $P$-$K$. Tutti i problemi trattati nelle lezioni precedenti possono essere riscritti come:
$$
\begin{bmatrix}
z\\
v
...
z=N(K)w.
$$
La mappa chiusa \`e una lower LFT:
$$
N(K)=F_\ell(P,K)
=
P_{11}+P_{12}K(I-P_{22}K)^{-1}P_{21}.
$$
Questa formula \`e il ponte fra schemi a blocchi, pesi di progetto e algoritmi di sintesi $\mathcal{H}_2/\mathcal{H}_\infty$.

\begin{keybox}
\textbf{Messaggio operativo.} Prima di usare una routine di sintesi bisogna fare il lavoro concettuale: scegliere bene $w$, $z$, $v$ e $u$, costruire $P$ aprendo l'anello sul controllore, e verificare che la chiusura $F_\ell(P,K)$ produca davvero lo stack di prestazione desiderato.
\end{keybox}
...
Questa parte del documento \`e una traduzione e rielaborazione estesa delle pagine 1--24 del PDF ``General Control Problem Formulation -- $\mathcal{H}_2/\mathcal{H}_\infty$ control synthesis'', caricato dall'utente come materiale di partenza nella directory \texttt{Slide}.

% --- FRONTESPIZIO LEZIONE 17 ---
\begin{coverpage}
\centering
{\Huge\bfseries Controllo Robusto\par}
\vspace{0.4em}
{\LARGE Traduzione in italiano e approfondimento\par}
\vspace{0.5em}
{\Large Sintesi ottima $\mathcal{H}_2/\mathcal{H}_\infty$, realizzazione generalizzata e principio di separazione\par}
\vfill
\includegraphics[width=0.78\textwidth]{images17/h2hinf-25-42-007.png}
\vfill
\begin{focusbox}
\textbf{Obiettivo del documento.} Questa parte continua la formulazione generale $P$-$K$. Prima si mostra come ricavare una pianta generalizzata $P$ a partire da una mappa chiusa desiderata $N(K)$; poi si passa alla realizzazione in spazio di stato, alla definizione dei problemi ottimi $\mathcal{H}_2$ e $\mathcal{H}_\infty$, alla struttura del controllore dinamico e al principio di separazione fra feedback di stato e osservatore.
\end{focusbox}
\vfill
{\large Basato sulle slide del corso AA 2025/2026\par}
\end{coverpage}
...
La Lezione 16 ha introdotto la forma generale del problema di controllo:
$$
\begin{bmatrix}
z\\
v
...
N(K)=F_\ell(P,K).
$$
La Lezione 17 riparte da qui e rende questa formulazione pi\`u operativa.

Il percorso delle slide 25--42 \`e il seguente:
\begin{enumerate}
    \item ricavare una possibile pianta generalizzata $P$ partendo da uno stack $N(K)$ gi\`a noto;
    \item riconoscere le diverse piante $P$ associate a problemi $S/KS$, $S/T/KS$ e \texttt{mixsyn};
    \item scrivere $P$ in forma di stato e identificare le matrici $A,B_1,B_2,C_1,C_2,D_{ij}$;
    \item definire i problemi ottimi e subottimi $\mathcal{H}_2$ e $\mathcal{H}_\infty$;
    \item descrivere la struttura del controllore dinamico basato su osservatore e feedback di stato;
    \item arrivare al principio di separazione.
\end{enumerate}

\begin{keybox}
\textbf{Idea guida.} La pianta generalizzata non \`e solo un diagramma a blocchi comodo: \`e l'oggetto matematico che entra negli algoritmi di sintesi. Scegliere $P$ significa scegliere che cosa il controllore deve vedere, che cosa pu\`o comandare e quali uscite pesate devono diventare piccole.
\end{keybox}

\subsection{Ricavare \texorpdfstring{$P$}{P} da \texorpdfstring{$N$}{N}: sensibilit\`a pesata}

La prima slide riprende la procedura inversa introdotta alla fine della lezione precedente: dato uno stack chiuso $N(K)$, vogliamo trovare una pianta generalizzata $P$ tale che
$$
N(K)=F_\ell(P,K)
=
P_{11}+P_{12}K(I-P_{22}K)^{-1}P_{21}.
$$
L'esempio pi\`u semplice \`e la sensibilit\`a pesata:
$$
N(K)=w_P S=w_P(I+GK)^{-1},
$$
dove $w_P$ \`e un peso scalare.

Il primo blocco si ottiene ponendo $K=0$:
$$
P_{11}=N(0)=w_P I.
$$
Poi si definisce
...
=-w_PGK(I+GK)^{-1}.
$$
Il fattore comune da riconoscere \`e
$$
R=K(I+GK)^{-1}.
$$
Per far comparire questo stesso denominatore nella LFT bisogna scegliere
$$
P_{22}=-G,
$$
perch\'e
$$
K(I-P_{22}K)^{-1}=K(I+GK)^{-1}.
$$
A questo punto
$$
Q=-w_PG R,
$$
quindi una scelta naturale \`e
$$
P_{12}=-w_PG,
\qquad
P_{21}=I.
...
\begin{focusbox}
\textbf{Lettura del risultato.} La riga superiore costruisce l'uscita pesata $z$; la riga inferiore costruisce il segnale $v$ dato al controllore. La scelta $P_{22}=-G$ codifica il denominatore chiuso $I+GK$.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-001.png}
    \caption{Procedura inversa applicata alla sensibilit\`a pesata: da $N=w_PS$ si ricava una pianta generalizzata compatibile.}
\end{figure}

\subsection{Ricavare \texorpdfstring{$P$}{P} da uno stack di mixed sensitivity}

La slide successiva applica la stessa idea allo stack
$$
N(K)=
\begin{bmatrix}
W_uKS\\
...
Poich\'e
$$
W_P(S-I)=-W_PGK(I+GK)^{-1},
$$
si vede ancora il fattore comune
$$
R=K(I+GK)^{-1},
\qquad
P_{22}=-G.
...
P_{21}=I.
$$
La pianta generalizzata diventa
$$
P=
\begin{bmatrix}
0 & W_u\\
...
Questa forma \`e leggermente diversa da altre piante generalizzate viste in precedenza, ma rappresenta lo stesso tipo di specifica. Nei problemi di norma, cambiare alcuni segni o distribuire fattori fra $P_{12}$ e $P_{21}$ pu\`o non cambiare il valore della norma chiusa. La cosa importante \`e che la LFT finale produca lo stack desiderato.

\begin{keybox}
\textbf{Non unicit\`a di $P$.} Dato $N(K)$, i blocchi $P_{11}$ e $P_{22}$ sono vincolati dalla parte costante e dal denominatore della LFT. I blocchi $P_{12}$ e $P_{21}$ invece non sono unici: un fattore scalare o matriciale invertibile pu\`o essere spostato da un lato all'altro senza cambiare il prodotto.
\end{keybox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-002.png}
    \caption{Derivazione di $P$ a partire dallo stack $[W_uKS\ \ W_TT\ \ W_PS]^T$.}
\end{figure}

\subsection{Esempio con disturbo come ingresso esogeno}

Le slide tornano poi a una costruzione pi\`u fisica, partendo dal diagramma a blocchi. Nel primo esempio l'unico ingresso esogeno \`e un disturbo
$$
w=d,
$$
e il riferimento \`e posto a zero. Si scelgono due uscite di prestazione:
$$
z=
\begin{bmatrix}
z_1\\
...
z_2=-W_2u.
$$
Il segnale misurato dal controllore \`e
$$
v=-y.
$$
Con
...
y=Gu+d,
$$
la pianta generalizzata che manda $[w\ u]^T$ in $[z_1\ z_2\ v]^T$ \`e
$$
\begin{bmatrix}
z_1\\
z_2\\
...
\end{bmatrix}.
$$
Il primo canale pesa l'effetto del disturbo sull'uscita, mentre il secondo pesa lo sforzo di controllo necessario a rigettarlo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-003.png}
    \caption{Problema $S/KS$ in regolazione: il disturbo $d$ \`e l'ingresso esogeno e il riferimento \`e nullo.}
\end{figure}

\subsection{Esempio con riferimento come ingresso esogeno}

Nel secondo esempio l'ingresso esogeno \`e il riferimento:
$$
w=r.
$$
Si considera l'errore
$$
e=r-y,
$$
e si scelgono
...
v=e=r-y.
$$
Poich\'e $y=Gu$, la pianta generalizzata \`e
$$
\begin{bmatrix}
z_1\\
z_2\\
...
\end{bmatrix}.
$$
La differenza rispetto al caso precedente \`e nei segni e nel significato di $w$: ora il controllore deve inseguire un riferimento, non rigettare un disturbo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-004.png}
    \caption{Problema $S/KS$ in tracking: il riferimento $r$ \`e l'ingresso esogeno.}
\end{figure}

\subsection{Aggiungere \texorpdfstring{$T$}{T}: il problema risolto da \texttt{mixsyn}}

La slide successiva aggiunge anche la sagomatura della funzione di sensibilit\`a complementare. Si prendono
$$
w=r,
\qquad
v=e=r-y,
$$
e si definiscono tre uscite di prestazione:
$$
z_1=W_1e=W_1S w,
\qquad
z_2=W_2u=W_2KS w,
...
z_3=W_3y=W_3T w.
$$
Questo \`e il problema tipico risolto dal comando Matlab \texttt{mixsyn}: si vuole rendere piccola $S$ dove conta la precisione, limitare $KS$ dove conta lo sforzo di controllo, e rendere piccola $T$ dove contano rumore e robustezza ad alta frequenza.

La pianta generalizzata fra ingressi $[w\ u]^T$ e uscite $[z_1\ z_2\ z_3\ v]^T$ \`e
$$
\begin{bmatrix}
z_1\\
z_2\\
...
\begin{focusbox}
\textbf{Collegamento con i pesi.} In questa notazione $W_1$ \`e il peso sulla sensibilit\`a, $W_2$ il peso sul comando e $W_3$ il peso sulla sensibilit\`a complementare. La scelta dei pesi determina il compromesso fra tracking, uso degli attuatori e attenuazione del rumore.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-005.png}
    \caption{Tracking con pesi su $S$ e $KS$: la forma $P$-$K$ esplicita ingressi e uscite usate da Matlab.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-006.png}
    \caption{Problema $S/KS/T$ associato a \texttt{mixsyn}: lo stack chiuso \`e $[W_1S\ \ W_2KS\ \ W_3T]^T$.}
\end{figure}

\subsection{Realizzazione della pianta generalizzata}

Dopo gli esempi a blocchi, le slide passano alla forma di stato. Nel problema generale si considera una pianta generalizzata LTI propria e razionale:
$$
\begin{bmatrix}
z(s)\\
v(s)
...
Qui:
\begin{itemize}
    \item $w$ raccoglie gli ingressi esogeni, spesso gi\`a normalizzati o pesati;
    \item $z$ raccoglie le uscite di prestazione da rendere piccole;
    \item $v$ raccoglie i segnali disponibili al controllore;
    \item $u$ \`e il comando generato dal controllore.
\end{itemize}

Una realizzazione in spazio di stato di $P$ \`e
$$
\begin{aligned}
\dot{x}(t)&=Ax(t)+B_1w(t)+B_2u(t),\\
z(t)&=C_1x(t)+D_{11}w(t)+D_{12}u(t),\\
v(t)&=C_2x(t)+D_{21}w(t)+D_{22}u(t).
\end{aligned}
$$
In forma compatta:
$$
...
\right].
$$
Questa realizzazione spesso contiene la pianta fisica aumentata con pesi dinamici. Per questo le dimensioni degli stati di $P$ possono essere maggiori delle dimensioni degli stati della sola pianta nominale.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-007.png}
    \caption{Forma generale della pianta $P$: ingressi esogeni, comandi, uscite di prestazione e misure disponibili.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-008.png}
    \caption{Significato dei segnali $w,z,v,u$ nella formulazione generale del problema di sintesi.}
\end{figure}

\subsection{Dal modello di stato alla matrice di trasferimento}

Applicando la trasformata di Laplace all'equazione di stato, con condizioni iniziali nulle, si ottiene
$$
sx(s)=Ax(s)+B_1w(s)+B_2u(s),
$$
quindi
$$
x(s)=(sI-A)^{-1}B_1w(s)+(sI-A)^{-1}B_2u(s).
$$
Sostituendo nelle equazioni di uscita:
$$
\begin{bmatrix}
z(s)\\
v(s)
...
\end{bmatrix}.
$$
Questa \`e la forma che collega la notazione a blocchi della LFT alla rappresentazione usata dagli algoritmi numerici.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-009.png}
    \caption{Dalla realizzazione in spazio di stato alla matrice di trasferimento della pianta generalizzata.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-010.png}
    \caption{Partizione di $P(s)$ nei blocchi $P_{11},P_{12},P_{21},P_{22}$ e relativo diagramma a blocchi.}
\end{figure}

\subsection{Problemi ottimi \texorpdfstring{$\mathcal{H}_2$}{H2} e \texorpdfstring{$\mathcal{H}_\infty$}{Hinf}}

Una volta fissata la pianta generalizzata $P$, la sintesi consiste nel scegliere un controllore proprio, reale e razionale $K(s)$ che chiuda l'anello in modo ammissibile. Qui \emph{ammissibile} significa almeno che il sistema chiuso sia ben posto e internamente stabile.

La mappa chiusa da $w$ a $z$ \`e
$$
T_{zw}(s)=F_\ell(P(s),K(s)).
$$
Il problema ottimo $\mathcal{H}_2$ consiste nel trovare $K(s)$ che minimizza
$$
\|T_{zw}\|_2
=
\|F_\ell(P,K)\|_2.
$$
Questa norma misura una prestazione energetica media: \`e naturale quando gli ingressi esogeni sono interpretati come rumori bianchi o segnali stocastici.

Il problema ottimo $\mathcal{H}_\infty$ consiste invece nel trovare $K(s)$ che minimizza
$$
\|T_{zw}\|_\infty
=
\|F_\ell(P,K)\|_\infty.
$$
Il valore minimo viene indicato con
$$
\gamma_{\mathrm{opt}}.
$$
Questa norma misura la massima amplificazione in frequenza:
$$
\|T_{zw}\|_\infty
=
\sup_\omega \bar{\sigma}\!\left(T_{zw}(j\omega)\right).
$$
Per sistemi stabili, essa coincide anche con il guadagno indotto $L_2$:
$$
\|T_{zw}\|_\infty
=
\sup_{w\neq0}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-011.png}
    \caption{Definizione dei problemi ottimi $\mathcal{H}_2$ e $\mathcal{H}_\infty$ nella forma $P$-$K$.}
\end{figure}

\subsection{Problema subottimo \texorpdfstring{$\mathcal{H}_\infty$}{Hinf}}

In pratica trovare esattamente $\gamma_{\mathrm{opt}}$ pu\`o essere difficile. Per questo si formula spesso il problema subottimo:

\begin{quote}
Dato un valore $\gamma>\gamma_{\mathrm{opt}}$, trovare un controllore ammissibile $K(s)$ tale che
$$
\|T_{zw}\|_\infty
=
\|F_\ell(P,K)\|_\infty
...
Questo modo di ragionare \`e molto importante negli algoritmi $\mathcal{H}_\infty$. Invece di chiedere subito il migliore valore possibile, si fissa una soglia $\gamma$ e si verifica se esiste un controllore che la soddisfa. Poi si riduce progressivamente $\gamma$ fino ad avvicinarsi al valore ottimo.

\begin{focusbox}
\textbf{Interpretazione di $\gamma$.} Se $\|T_{zw}\|_\infty\leq\gamma$, allora nessun ingresso esogeno $w$ di energia finita pu\`o produrre un'uscita di prestazione $z$ con energia amplificata pi\`u di $\gamma$. Il progetto $\mathcal{H}_\infty$ \`e quindi una garanzia worst-case.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-012.png}
    \caption{Problema subottimo $\mathcal{H}_\infty$: dato $\gamma$, si cerca un controllore con norma chiusa non superiore a $\gamma$.}
\end{figure}

\subsection{Struttura del controllore: osservatore e feedback di stato}

Le slide introducono poi la struttura tipica del controllore dinamico. L'idea \`e combinare due sottosistemi:
\begin{enumerate}
    \item un osservatore che ricostruisce una stima $\hat{x}(t)$ dello stato;
    \item una legge di feedback di stato applicata alla stima.
\end{enumerate}

Per evitare ambiguit\`a, indichiamo con $K_x$ il guadagno di feedback di stato, mentre $K(s)$ resta la matrice di trasferimento del controllore. L'osservatore di Luenberger ha la forma
$$
\dot{\hat{x}}(t)
=
A\hat{x}(t)+B_2u(t)+L\bigl(v(t)-C_2\hat{x}(t)\bigr),
$$
dove $L$ \`e il guadagno dell'osservatore. La legge di controllo \`e
$$
u(t)=-K_x\hat{x}(t).
$$
Sostituendo nella dinamica dell'osservatore:
$$
\dot{\hat{x}}(t)
=
\left(A-B_2K_x-LC_2\right)\hat{x}(t)+Lv(t).
$$
In Laplace:
$$
\hat{x}(s)
=
\left(sI-A+B_2K_x+LC_2\right)^{-1}Lv(s),
$$
e dunque
$$
u(s)
...
K(s)
=
-K_x\left(sI-A+B_2K_x+LC_2\right)^{-1}L.
$$

\begin{keybox}
\textbf{Punto delicato.} Le matrici $A,B_2,C_2$ usate qui sono quelle della pianta generalizzata, non necessariamente quelle della sola pianta fisica. Se i pesi dinamici aggiungono stati, anche il controllore li vede nella realizzazione aumentata.
\end{keybox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-013.png}
    \caption{Struttura del controllore: osservatore di stato pi\`u feedback sulla stima $\hat{x}$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-014.png}
    \caption{Funzione di trasferimento del controllore dinamico ottenuta dalla struttura osservatore-feedback.}
\end{figure}

\subsection{Matrice di trasferimento chiusa}

Consideriamo ora un controllore dinamico generico
$$
\begin{aligned}
\dot{x}_K(t)&=A_Kx_K(t)+B_Kv(t),\\
u(t)&=C_Kx_K(t)+D_Kv(t).
\end{aligned}
$$
Usiamo, come nelle slide successive, il caso standard
$$
D_{11}=0,
\qquad
D_{22}=0,
...
\end{bmatrix}.
$$
Sostituendo le equazioni del controllore in quelle della pianta si ottiene una realizzazione chiusa
$$
\begin{aligned}
\dot{x}_{\mathrm{cl}}(t)&=A_{\mathrm{cl}}x_{\mathrm{cl}}(t)+B_{\mathrm{cl}}w(t),\\
z(t)&=C_{\mathrm{cl}}x_{\mathrm{cl}}(t)+D_{\mathrm{cl}}w(t),
\end{aligned}
$$
con
$$
...
$$
$$
D_{\mathrm{cl}}=D_{12}D_KD_{21}.
$$
La matrice di trasferimento chiusa \`e quindi
$$
T_{zw}(s)
=
C_{\mathrm{cl}}(sI-A_{\mathrm{cl}})^{-1}B_{\mathrm{cl}}
+D_{\mathrm{cl}}.
$$

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-015.png}
    \caption{Controllore dinamico generale e costruzione della realizzazione chiusa da $w$ a $z$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-016.png}
    \caption{Matrici $A_{\mathrm{cl}},B_{\mathrm{cl}},C_{\mathrm{cl}},D_{\mathrm{cl}}$ e trasferimento chiuso $T_{zw}$.}
\end{figure}

\subsection{Caso osservatore-feedback}

Per un controllore basato su osservatore si ha
$$
D_K=0,
\qquad
A_K=A-B_2K_x-LC_2,
...
D_{\mathrm{cl}}=0.
$$
Questa forma mette in evidenza che il sistema chiuso contiene due dinamiche accoppiate: la dinamica dello stato reale $x$ e la dinamica della stima $\hat{x}$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-017.png}
    \caption{Realizzazione chiusa nel caso di controllore basato su osservatore, con $D_K=0$.}
\end{figure}

\subsection{Principio di separazione}

L'ultima slide mostra che, con un cambiamento di coordinate, la struttura osservatore-feedback diventa triangolare. Definiamo l'errore di stima
$$
e(t)=x(t)-\hat{x}(t),
$$
e usiamo il nuovo stato
$$
x_n(t)=
\begin{bmatrix}
x(t)\\
...
\end{bmatrix}.
$$
La trasformazione fra le coordinate originali $[x\ \hat{x}]^T$ e le nuove coordinate \`e
$$
\begin{bmatrix}
x\\
\hat{x}
...
P_t^{-1}=P_t.
$$
Nelle nuove coordinate la matrice di stato diventa
$$
A_n
=
P_t^{-1}A_{\mathrm{cl}}P_t
=
\begin{bmatrix}
A-B_2K_x & B_2K_x\\
0 & A-LC_2
\end{bmatrix}.
$$
La matrice \`e triangolare a blocchi, quindi i suoi autovalori sono l'unione degli autovalori dei due blocchi diagonali:
$$
\lambda(A_n)
=
\lambda(A-B_2K_x)
...
\lambda(A-LC_2).
$$
Questo \`e il \emph{principio di separazione}: i poli scelti con il feedback di stato, assumendo lo stato disponibile, sono indipendenti dai poli scelti con l'osservatore, assumendo il controllore fissato.

Le altre matrici nelle nuove coordinate diventano, nella forma mostrata dalle slide,
$$
B_n=
\begin{bmatrix}
B_1\\
...
C_n=
\begin{bmatrix}
C_1-D_{12}K_x & D_{12}K_x
\end{bmatrix}.
$$

\begin{focusbox}
\textbf{Significato progettuale.} Per controllori osservatore-feedback, si pu\`o pensare separatamente a due problemi: scegliere $K_x$ per stabilizzare e sagomare la dinamica di controllo, e scegliere $L$ per rendere stabile e veloce la stima. Nel problema $\mathcal{H}_2/\mathcal{H}_\infty$ moderno questa idea resta utile, anche se le formule ottime sono pi\`u vincolate dalle prestazioni pesate.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images17/h2hinf-25-42-018.png}
    \caption{Principio di separazione: dopo il cambio di coordinate, la dinamica di feedback e quella dell'osservatore compaiono sui blocchi diagonali.}
\end{figure}

\subsection{Sintesi conclusiva della lezione}

La Lezione 17 rende concreta la formulazione generale. Da un lato, mostra che gli stack usati nelle lezioni precedenti possono essere riscritti come lower LFT di una pianta generalizzata $P$. Dall'altro, traduce la stessa idea in spazio di stato, preparando il terreno agli algoritmi di sintesi $\mathcal{H}_2$ e $\mathcal{H}_\infty$.

I punti principali sono:
\begin{itemize}
    \item da $N(K)$ si pu\`o ricostruire una possibile $P$, ma la scelta non \`e unica;
    \item esempi diversi, come regolazione e tracking, cambiano i segni e i segnali, ma producono gli stessi oggetti fondamentali $S$, $T$ e $KS$;
    \item la pianta generalizzata in forma di stato contiene modello, pesi e interconnessioni;
    \item il problema $\mathcal{H}_\infty$ subottimo cerca un controllore con $\|F_\ell(P,K)\|_\infty\leq\gamma$;
    \item un controllore osservatore-feedback combina stima dello stato e feedback sulla stima;
    \item il principio di separazione spiega perch\'e, in quella struttura, dinamica di controllo e dinamica di osservazione possono essere studiate come due blocchi distinti.
\end{itemize}

\begin{keybox}
\textbf{Messaggio operativo.} Prima di parlare di algoritmo $\mathcal{H}_2$ o $\mathcal{H}_\infty$, bisogna essere capaci di costruire la $P$ giusta. La qualit\`a del progetto dipende da come traduciamo requisiti, pesi, disturbi e misure nella mappa chiusa $w\mapsto z$.
\end{keybox}
...
Questa parte del documento \`e una traduzione e rielaborazione estesa delle pagine 25--42 del PDF ``General Control Problem Formulation -- $\mathcal{H}_2/\mathcal{H}_\infty$ control synthesis'', caricato dall'utente come materiale di partenza nella directory \texttt{Slide}.

% --- FRONTESPIZIO LEZIONE 18 ---
\begin{coverpage}
\centering
{\Huge\bfseries Controllo Robusto\par}
\vspace{0.4em}
{\LARGE Traduzione in italiano e approfondimento\par}
\vspace{0.5em}
{\Large Sintesi ottima $\mathcal{H}_2$, Riccati, output feedback e controllo LQG\par}
\vfill
\includegraphics[width=0.78\textwidth]{images18/h2hinf-43-77-001.png}
\vfill
\begin{focusbox}
\textbf{Obiettivo del documento.} Questa parte sviluppa la sintesi ottima $\mathcal{H}_2$ nella forma $P$-$K$. Si parte dal caso con stato completamente misurato, si ricava il problema di feedback di stato tramite equazioni di Riccati, si introduce il problema duale di stima, si combina tutto nell'output feedback ottimo e si mostra il collegamento con il controllo LQG, incluso il caso con azione integrale.
\end{focusbox}
\vfill
{\large Basato sulle slide del corso AA 2025/2026\par}
\end{coverpage}
...
La Lezione 17 ha chiuso la parte geometrica della formulazione generale: pianta generalizzata, controllore dinamico, mappa chiusa e principio di separazione. La Lezione 18 entra nel cuore della sintesi $\mathcal{H}_2$. L'idea \`e trasformare il problema di minimizzazione
$$
\min_K \|T_{zw}\|_2
=
\min_K\|F_\ell(P,K)\|_2
$$
in equazioni algebriche di Riccati.

Il percorso \`e ordinato in tre passaggi:
\begin{enumerate}
    \item prima si risolve il problema con stato disponibile, cio\`e il problema di \emph{state feedback};
    \item poi si risolve il problema duale di stima dello stato, cio\`e il progetto dell'osservatore;
    \item infine si combinano feedback e osservatore per ottenere l'output feedback ottimo $\mathcal{H}_2$ e il controllo LQG.
\end{enumerate}

\begin{keybox}
\textbf{Idea guida.} Nel caso $\mathcal{H}_2$ la separazione non \`e solo un'intuizione: il controllore ottimo si costruisce risolvendo due problemi di Riccati indipendenti, uno di regolazione e uno di stima.
\end{keybox}

\subsection{Norma \texorpdfstring{$\mathcal{H}_2$}{H2} e risposta impulsiva}

Consideriamo la pianta generalizzata nella forma standard
$$
\left[
\begin{array}{c}
\dot{x}\\
...
\right].
$$
Le ipotesi $D_{11}=0$ e $D_{22}=0$ sono tipiche nella presentazione teorica: evitano un passaggio diretto da $w$ a $z$ e un'algebra loop immediata fra $u$ e $v$.

Il problema $\mathcal{H}_2$ consiste nel trovare un controllore $K(s)$ che minimizzi
$$
\|T_{zw}(s)\|_2
=
\|F_\ell(P(s),K(s))\|_2.
$$
Se $T_{zw}$ ha $m$ ingressi e se $z_i(t)$ \`e la risposta impulsiva all'ingresso $i$-esimo, allora
$$
\|T_{zw}\|_2^2
=
\sum_{i=1}^{m}
\int_0^\infty z_i^T(t)z_i(t)\,dt.
$$
Quindi la norma $\mathcal{H}_2$ misura l'energia totale delle risposte impulsive da $w$ a $z$. \`E una misura naturale quando $w$ rappresenta disturbi energetici o rumori bianchi normalizzati.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-001.png}
    \caption{Problema ottimo $\mathcal{H}_2$: minimizzare la norma della mappa chiusa $T_{zw}=F_\ell(P,K)$.}
\end{figure}

\subsection{Problema \texorpdfstring{$\mathcal{H}_2$}{H2} con feedback di stato}

Cominciamo dal caso pi\`u semplice: lo stato $x(t)$ \`e completamente disponibile al controllore. In questo caso il segnale misurato \`e direttamente
$$
v=x,
$$
e si cerca una legge di controllo statica
$$
u(t)=-Kx(t).
$$
La pianta generalizzata usata per questo problema \`e
$$
\left[
\begin{array}{c}
\dot{x}\\
...
Le slide assumono:
\begin{enumerate}
    \item $(A,C_1)$ senza modi inosservabili sull'asse immaginario;
    \item $(A,B_2)$ stabilizzabile;
    \item $D_{12}^TD_{12}>0$;
    \item $D_{12}^TC_1=0$.
\end{enumerate}
Le ultime due ipotesi semplificano molto l'algebra: il peso sul comando \`e non singolare e non c'\`e termine incrociato fra stato e comando nell'uscita di prestazione.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-002.png}
    \caption{Sintesi $\mathcal{H}_2$ con stato disponibile: il controllore usa direttamente $x(t)$.}
\end{figure}

\subsection{Mappa chiusa con feedback di stato}

Sostituendo
$$
u=-Kx
$$
nelle equazioni di stato si ottiene
$$
\dot{x}
=
(A-B_2K)x+B_1w,
$$
e l'uscita di prestazione diventa
$$
z=(C_1-D_{12}K)x.
$$
In Laplace:
...
=
(C_1-D_{12}K)
\left(sI-A+B_2K\right)^{-1}B_1.
$$
Il problema \`e ora: scegliere $K$ in modo che $A-B_2K$ sia stabile e che la norma $\mathcal{H}_2$ di questa mappa sia minima.

Se $Y$ \`e il gramiano di controllabilit\`a del sistema chiuso, allora
$$
(A-B_2K)Y+Y(A-B_2K)^T+B_1B_1^T=0,
\qquad
Y=Y^T>0,
$$
e
...
=
\operatorname{tr}\!\left[
(C_1-D_{12}K)Y(C_1-D_{12}K)^T
\right].
$$
Questo porta al problema di ottimizzazione:
$$
\min_{K,Y}
\operatorname{tr}\!\left[
(C_1-D_{12}K)Y(C_1-D_{12}K)^T
\right]
$$
soggetto alla Lyapunov precedente.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-003.png}
    \caption{Con feedback di stato, la mappa $T_{zw}$ dipende da $A-B_2K$ e da $C_1-D_{12}K$.}
\end{figure}

\subsection{Formulazione duale con gramiano di osservabilit\`a}

La stessa norma pu\`o essere calcolata usando il gramiano di osservabilit\`a $X$. In questo caso
$$
(A-B_2K)^TX+X(A-B_2K)
+(C_1-D_{12}K)^T(C_1-D_{12}K)=0,
$$
con
$$
X=X^T>0,
...
\|T_{zw}\|_2^2
=
\operatorname{tr}\!\left(B_1^TXB_1\right).
$$
La formulazione duale \`e quella pi\`u comoda per ricavare la formula esplicita del guadagno ottimo $K$, perch\'e il guadagno entra direttamente nella Lyapunov dell'osservabilit\`a.

\begin{focusbox}
\textbf{Due modi di vedere lo stesso costo.} Con $Y$ si misura quanta energia entra negli stati dal disturbo $w$; con $X$ si misura quanta energia degli stati viene vista dall'uscita $z$. Per sistemi stabili i due calcoli danno la stessa norma $\mathcal{H}_2$.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-004.png}
    \caption{Formulazione equivalente con gramiano di osservabilit\`a $X$.}
\end{figure}

\subsection{Approccio con moltiplicatori di Lagrange}

Le slide ricavano il guadagno ottimo usando un approccio a moltiplicatori di Lagrange. Si considera l'ottimizzazione con gramiano $X$ e si introduce una matrice simmetrica di moltiplicatori $\Lambda=\Lambda^T$. L'Hamiltoniana scalare \`e, in forma compatta,
$$
\mathcal{H}
=
\operatorname{tr}(B_1^TXB_1)
+
\operatorname{tr}
\left[
\Lambda
\left(
(A-B_2K)^TX+X(A-B_2K)
+(C_1-D_{12}K)^T(C_1-D_{12}K)
\right)
\right].
$$
Le condizioni necessarie rispetto a $K$, $X$ e $\Lambda$ producono:
$$
\frac{\partial\mathcal{H}}{\partial K}=0,
\qquad
\frac{\partial\mathcal{H}}{\partial X}=0,
\qquad
\frac{\partial\mathcal{H}}{\partial\Lambda}=0.
$$
La terza condizione recupera la Lyapunov di osservabilit\`a. La prima condizione d\`a, usando $D_{12}^TC_1=0$,
$$
-B_2^TX+D_{12}^TD_{12}K=0.
$$
Quindi
$$
K=
\left(D_{12}^TD_{12}\right)^{-1}B_2^TX.
$$
Questa \`e la formula attesa: il feedback \`e proporzionale al gramiano di osservabilit\`a ottimo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-005.png}
    \caption{Impostazione tramite moltiplicatori di Lagrange per il problema $\mathcal{H}_2$ con feedback di stato.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-006.png}
    \caption{Condizioni necessarie e formula del guadagno $K$ in funzione della soluzione $X$.}
\end{figure}

\subsection{Equazione algebrica di Riccati per il feedback}
...
\left(D_{12}^TD_{12}\right)^{-1}B_2^TX
$$
nell'equazione di Lyapunov si ottiene l'equazione algebrica di Riccati
$$
A^TX+XA
-XB_2\left(D_{12}^TD_{12}\right)^{-1}B_2^TX
+C_1^TC_1=0.
$$
La soluzione cercata \`e quella semidefinita positiva che rende stabile
$$
A-B_2K.
$$
La matrice Hamiltoniana associata \`e
$$
H_X=
\begin{bmatrix}
A & -B_2\left(D_{12}^TD_{12}\right)^{-1}B_2^T\\
-C_1^TC_1 & -A^T
\end{bmatrix}.
$$
Questa matrice \`e importante perch\'e le soluzioni stabilizzanti dell'equazione di Riccati possono essere calcolate a partire dal suo sottospazio invariante stabile.

\begin{keybox}
\textbf{Messaggio operativo.} Il problema $\mathcal{H}_2$ con stato misurato si riduce a una Riccati. Una volta trovata $X$, il controllore \`e semplicemente $u=-Kx$ con $K=(D_{12}^TD_{12})^{-1}B_2^TX$.
\end{keybox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-007.png}
    \caption{Equazione algebrica di Riccati e matrice Hamiltoniana per il feedback di stato.}
\end{figure}

\subsection{Collegamento con il problema LQ}

Le slide mostrano poi che il problema $\mathcal{H}_2$ con feedback di stato contiene il problema classico lineare-quadratico. Consideriamo una pianta
$$
\dot{x}=Ax+Bu+Tw_d,
\qquad
y=Cx+w_n.
$$
Nel problema LQ si assume lo stato disponibile e si vuole minimizzare
$$
J_{\mathrm{LQ}}
=
\lim_{T\to\infty}
...
R=R^T>0.
$$
Si costruisce allora una pianta generalizzata con
$$
z=
\begin{bmatrix}
Q^{1/2}Mx\\
...
D_{12}^TD_{12}=R.
$$
La Riccati $\mathcal{H}_2$ diventa la Riccati LQR:
$$
A^TX+XA-XBR^{-1}B^TX+M^TQM=0,
$$
e
$$
K=R^{-1}B^TX.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-008.png}
    \caption{Il problema $\mathcal{H}_2$ con feedback di stato si riduce al classico problema LQ.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-009.png}
    \caption{Pianta generalizzata per il costo LQ: si pesano combinazioni di stato e comando.}
\end{figure}

\subsection{Esempio: doppio integratore}

Le slide propongono un esempio elementare:
$$
\dot{x}_1=x_2,
\qquad
\dot{x}_2=u+d=u+w.
...
\end{bmatrix}w.
$$
Si assume che lo stato sia disponibile per il controllo. L'uscita virtuale da mantenere piccola \`e composta dalla posizione e dal comando:
$$
z=
\begin{bmatrix}
x_1\\
...
\end{bmatrix}.
$$
La Riccati restituisce la soluzione stabilizzante
$$
X=
\begin{bmatrix}
\sqrt{2} & 1\\
...
\end{bmatrix}.
$$
La legge di controllo ottima \`e quindi
$$
u(t)=-x_1(t)-\sqrt{2}\,x_2(t).
$$
Il segno \`e quello della convenzione $u=-Kx$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-010.png}
    \caption{Esempio di doppio integratore: modello in forma di stato e stato disponibile al controllore.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-011.png}
    \caption{Uscita virtuale $z=[x_1\ u]^T$ e matrici della pianta generalizzata per il doppio integratore.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-012.png}
    \caption{Risoluzione della Riccati per l'esempio del doppio integratore.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-013.png}
    \caption{Guadagno ottimo $K=[1\ \sqrt{2}]$ per il doppio integratore.}
\end{figure}

\subsection{Problema duale: stima dello stato}

Passiamo ora al problema di stima. Qui non vogliamo direttamente scegliere $u$, ma ricostruire lo stato $x(t)$ a partire da misure rumorose. Si considera
$$
\dot{x}=Ax+B_1w+B_2u,
\qquad
y=C_2x+D_{21}w.
...
\dot{\hat{x}}
=
A\hat{x}+B_2u+L(y-\hat{y}),
\qquad
\hat{y}=C_2\hat{x}.
$$
Il guadagno $L$ deve essere scelto in modo da rendere stabile l'errore di stima e minimizzare l'influenza di $w$ su tale errore.
...
\dot{e}
=
(A-LC_2)e+(B_1-LD_{21})w.
$$
Se l'uscita da rendere piccola \`e
$$
z=C_1e,
$$
la mappa da $w$ a $z$ \`e
...
T_{zw}(s)
=
C_1(sI-A+LC_2)^{-1}(B_1-LD_{21}).
$$
Il problema di stima $\mathcal{H}_2$ consiste nel trovare $L$ tale che $A-LC_2$ sia Hurwitz e $\|T_{zw}\|_2$ sia minima.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-014.png}
    \caption{Problema di stima: l'osservatore usa $y$ per ricostruire lo stato $x$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-015.png}
    \caption{Dinamica dell'errore di stima e trasferimento da $w$ a $z=C_1e$.}
\end{figure}

\subsection{Sintesi \texorpdfstring{$\mathcal{H}_2$}{H2} dello stimatore}

Usando il gramiano di osservabilit\`a $Y$ della mappa di errore, la norma pu\`o essere scritta come
$$
\|T_{zw}\|_2^2
=
\operatorname{tr}\!\left[
...
dove
$$
(A-LC_2)^TY+Y(A-LC_2)+C_1^TC_1<0.
$$
In modo duale, con il gramiano di controllabilit\`a $X_e$:
$$
(A-LC_2)X_e+X_e(A-LC_2)^T
+(B_1-LD_{21})(B_1-LD_{21})^T<0,
$$
e
$$
\|T_{zw}\|_2^2
=
\operatorname{tr}(C_1X_eC_1^T).
$$
La forma \`e la stessa del feedback di stato, ma con matrici trasposte e ruoli scambiati. Questo \`e il principio di dualit\`a.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-016.png}
    \caption{Problema $\mathcal{H}_2$ di stima scritto con il gramiano di osservabilit\`a.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-017.png}
    \caption{Dualit\`a fra problema di feedback di stato e problema di stima.}
\end{figure}

\subsection{Riccati per lo stimatore}

La pianta generalizzata per il problema di stima si ottiene per dualit\`a:
$$
\left[
\begin{array}{c}
\dot{x}\\
...
\right].
$$
Le ipotesi duali sono:
\begin{enumerate}
    \item $(A,B_1)$ senza modi incontrollabili sull'asse immaginario;
    \item $(A,C_2)$ rilevabile;
    \item $D_{21}D_{21}^T>0$;
    \item $B_1D_{21}^T=0$.
\end{enumerate}
Il guadagno ottimo dello stimatore \`e
$$
L=YC_2^T(D_{21}D_{21}^T)^{-1},
$$
dove $Y=Y^T\geq0$ risolve
$$
AY+YA^T
-YC_2^T(D_{21}D_{21}^T)^{-1}C_2Y
+B_1B_1^T=0.
$$
La matrice Hamiltoniana associata pu\`o essere scritta come
$$
H_Y=
\begin{bmatrix}
A^T & -C_2^T(D_{21}D_{21}^T)^{-1}C_2\\
-B_1B_1^T & -A
\end{bmatrix}.
$$
La soluzione stabilizzante \`e quella che rende stabile
$$
A-LC_2.
$$
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-018.png}
    \caption{Pianta generalizzata e ipotesi per il problema $\mathcal{H}_2$ di stima.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-019.png}
    \caption{Guadagno dello stimatore e Riccati duale per la matrice $Y$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-020.png}
    \caption{Hamiltoniana del problema di stima e valore minimo del costo.}
\end{figure}

\subsection{Output feedback ottimo \texorpdfstring{$\mathcal{H}_2$}{H2}}

Le slide arrivano poi al caso completo: lo stato non \`e misurato, quindi il controllore deve stimarlo e usare la stima per controllare. Il risultato \`e notevole:

\begin{quote}
Il problema $\mathcal{H}_2$ di output feedback si separa in due problemi LQ indipendenti: uno di feedback di stato e uno di stima.
\end{quote}

La pianta generale \`e
$$
\left[
\begin{array}{c}
\dot{x}\\
...
Si assumono:
\begin{enumerate}
    \item assenza di modi incontrollabili e inosservabili sull'asse immaginario per la parte rilevante;
    \item $(A,B_2,C_2)$ stabilizzabile e rilevabile;
    \item $D_{12}^TD_{12}>0$ e $D_{21}D_{21}^T>0$;
    \item $D_{12}^TC_1=0$ e $B_1D_{21}^T=0$.
\end{enumerate}

Il controllore ottimo ha la forma osservatore-feedback:
$$
\dot{\hat{x}}
=
A\hat{x}+B_2u+L(v-C_2\hat{x}),
\qquad
u=-K\hat{x}.
$$
In forma di trasferimento:
$$
K_{\mathrm{of}}(s)
=
-K
\left(sI-A+B_2K+LC_2\right)^{-1}L.
$$
Equivalente realizzazione:
$$
\left[
\begin{array}{c}
\dot{\hat{x}}\\
...
I guadagni sono
$$
K=(D_{12}^TD_{12})^{-1}B_2^TX,
\qquad
L=YC_2^T(D_{21}D_{21}^T)^{-1},
$$
dove $X$ e $Y$ risolvono le due Riccati gi\`a viste:
$$
A^TX+XA-XB_2(D_{12}^TD_{12})^{-1}B_2^TX+C_1^TC_1=0,
$$
$$
AY+YA^T-YC_2^T(D_{21}D_{21}^T)^{-1}C_2Y+B_1B_1^T=0.
$$

\begin{keybox}
\textbf{Principio di separazione $\mathcal{H}_2$.} Il feedback ottimo $K$ e il guadagno di stima $L$ possono essere calcolati indipendentemente. Il controllore finale li combina nella struttura osservatore-feedback.
\end{keybox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-021.png}
    \caption{Il problema $\mathcal{H}_2$ di output feedback come combinazione di due problemi LQ.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-022.png}
    \caption{Ipotesi standard per il problema $\mathcal{H}_2$ di output feedback.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-023.png}
    \caption{Realizzazione del controllore ottimo $\mathcal{H}_2$ come osservatore pi\`u feedback sulla stima.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-024.png}
    \caption{Le due Riccati indipendenti e le Hamiltoniane associate.}
\end{figure}

\subsection{Valore minimo e ipotesi rilassate}

Il valore minimo del costo $\mathcal{H}_2$ pu\`o essere scritto come somma di due contributi: uno legato al feedback di stato e uno legato all'errore di stima. Con le convenzioni delle slide:
$$
\|T_{zw}\|_2^2
=
\operatorname{tr}(B_1^TXB_1)
+
\operatorname{tr}\!\left[
(D_{12}^TD_{12})K\,Y\,K^T
\right].
$$
Il primo termine \`e il costo che si avrebbe usando lo stato vero; il secondo misura il prezzo dovuto al fatto che si usa una stima.

Le ultime ipotesi di ortogonalit\`a,
$$
D_{12}^TC_1=0,
\qquad
B_1D_{21}^T=0,
$$
non sono indispensabili. Se non valgono, compaiono termini incrociati nelle Riccati.

Per il feedback di stato, ponendo
$$
R=D_{12}^TD_{12},
\qquad
S=C_1^TD_{12},
$$
la Riccati generale diventa
$$
A^TX+XA
-(XB_2+S)R^{-1}(B_2^TX+S^T)
+C_1^TC_1=0,
$$
e
$$
K=R^{-1}(B_2^TX+D_{12}^TC_1).
$$
Per lo stimatore, ponendo
$$
V=D_{21}D_{21}^T,
\qquad
N=B_1D_{21}^T,
...
$$
AY+YA^T
-(YC_2^T+N)V^{-1}(C_2Y+N^T)
+B_1B_1^T=0,
$$
e
$$
L=(YC_2^T+B_1D_{21}^T)V^{-1}.
$$

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-025.png}
    \caption{Valore minimo del costo $\mathcal{H}_2$ per il controllore di output feedback.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-026.png}
    \caption{Caso con ipotesi di ortogonalit\`a rilassate: compaiono termini incrociati nelle Riccati.}
\end{figure}

\subsection{Controllo Linear-Quadratic-Gaussian}

Le slide collegano ora la sintesi $\mathcal{H}_2$ al controllo LQG. Nel controllo LQG classico si assume che:
\begin{itemize}
    \item la dinamica sia lineare e nota;
    \item i disturbi di processo e i rumori di misura siano gaussiani, bianchi e con covarianze note;
    \item il costo sia quadratico.
\end{itemize}
La pianta \`e
$$
\dot{x}=Ax+Bu+Tw_d,
...
y=Cx+w_n,
$$
dove $w_d$ \`e il rumore di processo e $w_n$ il rumore di misura. Si assume, nel caso non correlato,
$$
\mathbb{E}
\left[
\begin{bmatrix}
...
R=R^T>0.
$$
Il nome LQG viene proprio da qui: modello \emph{lineare}, costo \emph{quadratico}, rumori \emph{gaussiani}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-027.png}
    \caption{Impostazione del controllo LQG: dinamica lineare, rumore di processo e rumore di misura gaussiani.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-028.png}
    \caption{Costo quadratico LQG e interpretazione del nome Linear-Quadratic-Gaussian.}
\end{figure}

\subsection{LQG come problema \texorpdfstring{$\mathcal{H}_2$}{H2}}

Il punto chiave \`e riscrivere LQG come un problema $\mathcal{H}_2$. Prima si definisce
$$
z=
\begin{bmatrix}
Q^{1/2}q\\
...
\end{bmatrix}.
$$
Poi si rappresentano i rumori come funzioni di un ingresso bianco normalizzato $w$:
$$
\begin{bmatrix}
w_d\\
w_n
...
\end{bmatrix}w.
$$
Con questa scelta, il costo LQG coincide con la norma $\mathcal{H}_2$ della mappa chiusa:
$$
J_{\mathrm{LQG}}
=
\|T_{zw}\|_2^2
...
\left[
\begin{array}{c|cc}
A & \begin{array}{cc}TW^{1/2} & 0\end{array} & B\\
\hline
\begin{array}{c}Q^{1/2}M\\0\end{array}
& 0
& \begin{array}{c}0\\R^{1/2}\end{array}\\
C & \begin{array}{cc}0 & V^{1/2}\end{array} & 0
\end{array}
\right]
\left[
\begin{array}{c}
...
\begin{focusbox}
\textbf{Traduzione concettuale.} LQG diventa $\mathcal{H}_2$ quando i rumori gaussiani vengono visti come ingressi bianchi normalizzati e il costo quadratico viene visto come energia dell'uscita virtuale $z$.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-029.png}
    \caption{Primo passo: definire l'uscita virtuale $z$ e normalizzare i rumori tramite le loro covarianze.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-030.png}
    \caption{Pianta generalizzata che riscrive LQG come problema $\mathcal{H}_2$.}
\end{figure}

\subsection{Guadagni LQG}

Il guadagno di feedback di stato, indicato nelle slide come $K_c$, \`e
$$
K_c=R^{-1}B^TX,
$$
dove $X=X^T>0$ risolve
$$
A^TX+XA-XBR^{-1}B^TX+M^TQM=0.
$$
Il guadagno dello stimatore, cio\`e il guadagno di Kalman, \`e
$$
K_f=YC^TV^{-1},
$$
dove $Y=Y^T>0$ risolve
$$
YA^T+AY-YC^TV^{-1}CY+TWT^T=0.
$$
La prima Riccati \`e quella del regolatore LQ; la seconda \`e quella del filtro di Kalman. Il controllore LQG \`e la combinazione delle due.

Le condizioni di esistenza richiedono stabilizzabilit\`a e rilevabilit\`a:
\begin{itemize}
    \item $(A,B,Q^{1/2}M)$ deve essere stabilizzabile/rilevabile nel senso richiesto dal problema LQ;
    \item $(A,TW^{1/2},C)$ deve essere stabilizzabile/rilevabile nel senso richiesto dal problema di stima.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-031.png}
    \caption{Riccati LQ e Riccati di Kalman: guadagni $K_c$ e $K_f$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-032.png}
    \caption{Condizioni di esistenza e metodo Hamiltoniano per risolvere le equazioni di Riccati.}
\end{figure}

\subsection{Metodo Hamiltoniano per la Riccati}

Le equazioni di Riccati possono essere risolte tramite matrici Hamiltoniane. Per una Riccati del tipo
$$
A^TP+PA-PBR^{-1}B^TP+Q=0,
$$
la matrice Hamiltoniana \`e
$$
H=
\begin{bmatrix}
A & -BR^{-1}B^T\\
...
\end{bmatrix}.
$$
Se $H$ non ha autovalori sull'asse immaginario, si ordinano gli autovettori in modo che i primi $n$ corrispondano agli autovalori a parte reale negativa:
$$
U=
\begin{bmatrix}
U_1\\
...
\end{bmatrix}.
$$
La soluzione stabilizzante della Riccati \`e
$$
P=U_2U_1^{-1}.
$$
Questa costruzione spiega perch\'e la scelta della soluzione giusta della Riccati non \`e arbitraria: serve quella associata al sottospazio stabile della Hamiltoniana.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-033.png}
    \caption{Soluzione della Riccati tramite sottospazio invariante stabile della matrice Hamiltoniana.}
\end{figure}

\subsection{Regolatore LQG}

Il regolatore LQG ha la struttura:
$$
\dot{\hat{x}}
=
...
u=-K_c\hat{x}.
$$
Sostituendo $u=-K_c\hat{x}$:
$$
\dot{\hat{x}}
=
(A-BK_c-K_fC)\hat{x}+K_fy,
$$
quindi una realizzazione del controllore \`e
$$
K_{\mathrm{LQG}}(s)
=
\left[
...
\right].
$$
Il controllore ha lo stesso grado della pianta: contiene un osservatore dello stato della pianta e applica su quello il feedback LQ.

\begin{keybox}
\textbf{LQG in una frase.} Il regolatore LQG \`e ``filtro di Kalman + LQR'': il filtro stima lo stato, l'LQR controlla la stima.
\end{keybox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-034.png}
    \caption{Regolatore LQG: stima ottima dello stato pi\`u feedback LQ sulla stima.}
\end{figure}

\subsection{LQG con azione integrale}

L'ultima slide mostra una variante molto usata in pratica: aggiungere azione integrale. L'obiettivo \`e migliorare il comportamento a regime, per esempio eliminare offset su riferimenti costanti o disturbi costanti.

Si costruisce una pianta aumentata introducendo uno stato integrale. Se lo stato integrale accumula l'uscita o l'errore, una forma tipica \`e
$$
x_a=
\begin{bmatrix}
x\\
...
\dot{\xi}=Cx.
$$
La dinamica aumentata diventa
$$
\dot{x}_a=A_ax_a+B_au+T_aw_d,
$$
con
$$
A_a=
...
\end{bmatrix}.
$$
Poi si applica lo stesso progetto LQG alla pianta aumentata. Il guadagno LQ agisce sia sullo stato stimato sia sullo stato integrale:
$$
K_a=
\begin{bmatrix}
K_c & K_i
\end{bmatrix}.
$$
Il controllore finale ha lo stesso ordine della pianta aumentata, quindi include sia l'osservatore sia la dinamica integrale.

\begin{focusbox}
\textbf{Perch\'e serve l'integratore.} Il LQG puro ottimizza una risposta media rispetto a rumori modellati, ma non garantisce automaticamente errore nullo a regime rispetto a segnali costanti. L'azione integrale aggiunge questa propriet\`a strutturale.
\end{focusbox}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{images18/h2hinf-43-77-035.png}
    \caption{Regolatore LQG con azione integrale: si progetta un LQG sulla pianta aumentata.}
\end{figure}

\subsection{Sintesi conclusiva della lezione}

La Lezione 18 costruisce il ponte fra la formulazione generale $P$-$K$ e gli algoritmi classici di sintesi ottima. Il problema $\mathcal{H}_2$ non resta una minimizzazione astratta: sotto ipotesi standard diventa una coppia di equazioni algebriche di Riccati.

I messaggi principali sono:
\begin{itemize}
    \item la norma $\mathcal{H}_2$ misura l'energia delle risposte impulsive da $w$ a $z$;
    \item con stato disponibile, il feedback ottimo \`e $u=-Kx$ e $K$ si ottiene da una Riccati;
    \item il problema di stima \`e il duale del problema di feedback di stato;
    \item l'output feedback ottimo $\mathcal{H}_2$ combina un feedback LQ e uno stimatore di Kalman;
    \item LQG \`e una formulazione probabilistica che si riscrive come problema $\mathcal{H}_2$;
    \item l'azione integrale si ottiene progettando il regolatore sulla pianta aumentata.
\end{itemize}

\begin{keybox}
\textbf{Messaggio operativo.} Quando un problema di controllo robusto viene portato nella forma generalizzata corretta, le scelte di costo, rumore, pesi e misure diventano matrici precise nelle Riccati. La teoria funziona bene solo se la costruzione di $P$ rappresenta davvero ci\`o che vogliamo ottimizzare.
\end{keybox}
...
Questa parte del documento \`e una traduzione e rielaborazione estesa delle pagine 43--77 del PDF ``General Control Problem Formulation -- $\mathcal{H}_2/\mathcal{H}_\infty$ control synthesis'', caricato dall'utente come materiale di partenza nella directory \texttt{Slide}.
